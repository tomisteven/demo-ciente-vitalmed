        ...
        <>
            <div className="tarjeta-turno">
                <div className="tarjeta-header">
                    <div className="fecha-hora">{formatearFechaHora(turno.fecha)}</div>
                </div>

                <div className="tarjeta-body">
                    <div className="info-item">
                        <span className="info-label">Médico:</span>
                        <span className="info-value">{turno.doctor?.nombre || "No especificado"}</span>
                    </div>

                    <div className="info-item">
                        <span className="info-label">Especialidad:</span>
                        <span className="info-value especialidad">{turno.especialidad || "No especificada"}</span>
                    </div>

                    {estudioAsignado && (
                        <div className="info-item">
                            <span className="info-label">Estudio:</span>
                            <span className="info-value estudio">{estudioAsignado}</span>
                        </div>
                    )}
                    ...
                    {turno.estudio?.precio > 0 && (
                        <div className="info-item">
                            ...
                            <span className="info-value precio">${turno.estudio.precio}</span>
                        </div>
                    )}
                </div>
                ...
                <div className="modal-overlay" onClick={handleCancelar}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="modal-body">
                            <p className="modal-info"><strong>Fecha:</strong> {formatearFechaHora(turno.fecha)}</p>
                            <p className="modal-info"><strong>Médico:</strong> {turno.doctor?.nombre}</p>
                        ...
                        <div className="modal-footer">
                            <button onClick={handleCancelar} className="btn-cancelar-modal" disabled={loading}>
                                Cancelar
                            </button>
                            <button onClick={handleConfirmar} className="btn-confirmar-modal" disabled={loading}>
